...
from ib_from_scratch.ib_utils import compute_upper_bound  # (get_entropy, get_kl_divergence, get_mutual_information, compute_upper_bound, get_joint_dist)
from ib_from_scratch.zas_ib_utils import H, DKL, MI, xlogx, joint
...
def element_log_loss(qw: np.array, pu_m: np.array, mwu: np.array, beta: float) -> float:
    ...
    ll_val = (np.log(qw[:, np.newaxis]) - beta * DKL(p=pu_m[:, np.newaxis, :], q=mwu[:, :, np.newaxis]))
    #ll_val = (np.log(qw) - beta * DKL(p=pu_m, q=mwu))
...
def update_pu(mwu, qw, encoder, meanings):
    ...
    print(f'Meanings: {meanings.shape}')
...
class IBModel:
    ...
    def __init__(self, observations, coordinates, meanings, words):
        ...
        #self.encoder = np.ones((words.shape[0], meanings.shape[0])) / words.shape[0]
    ...
    def run_single_annealing_iteration(self, beta: float = 1.0):
        ...
        print(f'Encoder size before update: {self.encoder.shape}')
        self.encoder, z = update_encoder(qw=self.qw, pu_m=self.pu_m, mwu=self.mwu, beta=beta)
        print(f'Encoder size after update: {self.encoder.shape}')

        # (from embo) remove unused dimensions
        self.qw, self.encoder, self.mwu = drop_unused_dimensions(self.qw, self.encoder, self.mwu, eps=0.001)
    ...
    def run_beta_specific_iterations_with_annealing(
        ...
        for a in range(attempts):
            ...
            while i < max_iter and not converged:
                ...
                try:
                    ...
                    # TODO: check if these references are right when defining joint dist
                    comp_joint = joint(self.encoder.T, self.pm)
                    acc_joint = joint(self.mwu.T, self.qw)
                    ...
                    ib_val = (MI(comp_joint) - beta * MI(acc_joint))
        ...
        if return_best:
            ...
            # TODO: check if these references are right when defining joint dist
            comp_joint = joint(best_enc.T, self.pm)
            acc_joint = joint(best_mwu.T, best_qw)
            check_best_res = {
                'MI meanings': MI(comp_joint),
                'MI obs': MI(acc_joint),
    ...
    def run_ib(self, n_beta: int, min_beta: float, max_beta: float, n_restarts: Optional[int], rtol: Optional[float],
        ...
        # TODO: check if these references are right when defining joint dist
        um_joint = joint(self.pu_m.T, self.pm)
        mi_um = MI(um_joint)
        max_ent = H(self.pm)
...
if __name__ == '__main__':
    ...
    res = ibm.run_ib(n_beta=5, min_beta=1.01, max_beta=1.10, n_restarts=3, rtol=1e-5, max_iter=500,
    ...
    # plot the IB bound
    plt_path = f'/Users/lindsayskinner/Documents/school/CLMS/Thesis/figures/ibplot_{vz}_A.png'
